...
function drawPlanetSphere(ctx, x, y, r, planet, isHovered, isSelected, t) {
  ...
  diffuse.addColorStop(0, 'rgba(255,252,240,0.28)');
...
document.getElementById('sidebarUp').addEventListener('click', () => {
  state.sidebarOffset = Math.max(0, state.sidebarOffset - 1);
  document.getElementById('sidebarScroll').style.transform =
    `translateY(${-state.sidebarOffset * SIDEBAR_ITEM_H}px)`;
});
document.getElementById('sidebarDown').addEventListener('click', () => {
...
//  CAMERA SYSTEM
// ─────────────────────────────────────
/* Camera transform formula (must be consistent everywhere):
   ctx.translate(CX, CY)
   ctx.scale(scale, scale)
   ctx.translate(-CX + camX + pxOff, -CY + camY + pyOff)

   Screen position of world point (wx, wy):
     sx = (wx - CX + camX + pxOff) * scale + CX
     sy = (wy - CY + camY + pyOff) * scale + CY
   For a planet at offset (p.px, p.py) from sun at (CX, CY):
     wx = CX + p.px  →  sx = (p.px + camX + pxOff) * scale + CX
*/
...
function selectPlanet(index) {
  ...
  // Target: planet moves to screen center
  // wx = CX + p.px  → sx = (p.px + camX) * scale + CX = CX
  //  ⟹  camX = -p.px
  const ts = 2.2;
...
function trackSelectedPlanet() {
  ...
  const ts = state.camera.targetScale;
  state.camera.targetX = -p.px;
  state.camera.targetY = -p.py;
}
...
function showPlanetPanel(planet, index) {
  ...
  document.getElementById('panelNumber').textContent   = `${num} / 08`;
  document.getElementById('panelName').textContent     = planet.name;
  document.getElementById('panelSubtitle').textContent = planet.subtitle;
  ...
  document.getElementById('panelDesc').textContent = planet.description;
  ...
  document.getElementById('planetPanel').classList.add('visible');
}

function hidePlanetPanel() {
  document.getElementById('planetPanel').classList.remove('visible');
}

document.getElementById('panelClose').addEventListener('click',  () => selectPlanet(null));
...
document.getElementById('btnPause').addEventListener('click', () => {
  ...
  document.getElementById('pauseIcon').style.display = state.paused ? 'none' : '';
  document.getElementById('playIcon').style.display  = state.paused ? ''     : 'none';
});

document.getElementById('btnReset').addEventListener('click', () => selectPlanet(null));
...
function updateSpeedHud() {
  ...
  document.getElementById('speedVal').textContent     = `${m}\u00d7`;
...
solarCanvas.addEventListener('mousemove', e => {
  ...
  if (hit !== null) {
    ...
    document.getElementById('tooltipName').textContent = p.name;
    document.getElementById('tooltipType').textContent = p.type;
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'Escape') {
    if (shortcutsOverlay.classList.contains('open')) { toggleShortcuts(false); return; }
    selectPlanet(null);
  }
  ...
  if (e.key === 'ArrowLeft'  && state.selectedPlanet !== null)
    document.getElementById('prevPlanet').click();
...
function render(ts) {
  ...
  if (!state.paused) {
    ...
    // Emit solar particles
    if (Math.random() < 0.35) emitSolarParticle();
  }

  // Track selected planet continuously
  trackSelectedPlanet();
  updateCamera();
  ...
  // Shooting stars (outside camera transform — screen space)
  ...
  // Sun sits at world-space center; parallax is already in ctx.translate above
  const sX = CX;
  const sY = CY;

  // Dim overlay for selected planet mode (fills entire transformed space)
  if (state.selectedPlanet !== null) {
    ctx.save();
  ...
  // Planets — sorted back to front (by y offset for perspective)
  ...
  sorted.forEach(pd => {
    ...
    // Rings behind planet (when planet is in lower half of orbit)
    ...
    // Rings in front (planet in upper half)
    if (pd.hasRings && pd.py < 0) {
      drawRings(ctx, px, py, visR, pd.ringColor, pd.ringThin);
    }
...
async function init() {
  ...
  // Start render loop immediately (runs behind loading screen)
  requestAnimationFrame(render);

  // Preload textures + show loading screen concurrently
  await Promise.all([
    preloadTextures(),
    new Promise(res => runLoadingScreen(res)),
  ]);

  // Rebuild sidebar AFTER textures ready → icons show real planet photos
  buildSidebar();

  // If '?explore=true' is in URL, auto-select a planet (e.g. Mercury or last visited)
  const urlParams = new URLSearchParams(window.location.search);
  if (urlParams.get('explore') === 'true') {
    let lastPlanet = localStorage.getItem('ep_last_planet');
    let indexToSelect = 0; // Default to Mercury
    if (lastPlanet) {
      const idx = state.planets.findIndex(p => p.id === lastPlanet);
      if (idx !== -1) indexToSelect = idx;
    }
